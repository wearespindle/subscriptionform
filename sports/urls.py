...
from .views import PerformanceCreate, PerformanceDelete, PerformanceUpdate
...
urlpatterns = [
    ...
    # New performance URL structure with a PK for participants and a PK for performances.
    url(r'participants/(?P<pk>[0-9]+)/performance/add/$', PerformanceCreate.as_view(), name='performance_add'),
    url(r'participants/(?P<pk>[0-9]+)/performance/(?P<performance_pk>\d+)/update/$', PerformanceUpdate.as_view(), name='performance_update'),
    url(r'participants/(?P<pk>[0-9]+)/performance/(?P<performance_pk>\d+)/delete/$', PerformanceDelete.as_view(), name='performance_delete'),
